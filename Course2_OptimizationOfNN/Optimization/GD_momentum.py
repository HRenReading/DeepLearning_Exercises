# -*- coding: utf-8 -*-
"""
Created on Sat May 11 13:09:31 2024
...
def GD_momentum(para,grad,v,beta,LR):
    ...
    for k in range(npara):
        # compute velocities
        v["dw"+str(k+1)] = beta*v["dw"+str(k+1)] + (1 - beta)*grad['dw'+str(k+1)]
        v["db"+str(k+1)] = beta*v["db"+str(k+1)] + (1 - beta)*grad['db'+str(k+1)]
...
def Mini_Batch_GDmomentum(x,y,layer_dim,n_epoch,batch_size,funhid,funout,gradfun,LR,beta):
    "Neural network using gradient descent with momentum"
    #initialize parameters
    para = initialization(layer_dim) 
    #initialize velocity for momentum  
    v = initial_velocity(layer_dim)  
    #initialize cost
